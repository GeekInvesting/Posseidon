<template>
  <div
    class="flex-center p-4 bg-white routed-lg shadow-md mx-5 md:mx-5 lg:mx-10 xl:mx-20"
  >
    <el-form
      class="mb-4"
      v-loading="loading"
...
const submit = () => {
  ...
  ElMessageBox.confirm(`Are you sure to ${props.investorSave} this investor?`, {
    ...
    .then(async () => {
      ...
      response
        ? PosseidonNotif(
            `success`,
            `Investor ${props.investorSave}d successfully`
          )
        : null;

      anotherInvestor.value ? emptyForm() : emitEventBus("dialogCreate", true);
